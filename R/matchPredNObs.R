...
#' @param var2junc string or data.frame A data.frame generated by mapVariantJunc function. One row in the the dataframe is an annotated DNA variant that mapped to a novel junction in a subject's tissues. 
...
function(gdb_path, cohort_name, var2junc, min_SpliceAI_score, max_skip_exon = 2, reference = "Default", SpliceAI_default_reference = FALSE){
  ...
  if (SpliceAI_default_reference == TRUE){
    ...
    if (sum(is.na(spliceai$spliceaiGENE)) > 0){
      warning(sprintf("Did not find gene ID for ", paste(spliceai[!is.na(spliceai$spliceaiGENE), "spliceaiSYMBOL"], sep=",")))
    }
