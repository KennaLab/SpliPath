...
#' @param max_LeafCutter_Pval numeric The threshold of LeafCutterMD P value of a novel junction.
#' @param min_nr_tissue numeric Require novel junction LeafCutterMD P value under the provided threshold in at least min_nr_tissue tissues.
...
annotateLeafCutterJunc <-
  function(rna_meta,
    ...
    if (sum(!tissues_leafcutter_ %in% rna_meta$Tissue) > 0){
      missing_tissue = tissues_leafcutter_[!tissues_leafcutter_ %in% rna_meta$Tissue]
    ...
    ### Merge the junctions
    message("Merging splicing junctions from multiple samples ...")
    ...
    if (!is.null(tissues_leafcutter_pvals_file)){
      ...
      leafcutter_pvals_pass_tissue_idx = rowSums(leafcutter_pvals_pass_tissue[, paste0("Outlier_P_", tissues_leafcutter_) ] < max_LeafCutter_Pval, na.rm=T) >= min_nr_tissue  
