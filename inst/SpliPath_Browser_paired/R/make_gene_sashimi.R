make_gene_wise_plot <- function(
  ...
  main_title = paste0(subject, " - ", exons_gene$gene.name[1])
  legend_title <- "Read"
  junction_colour <- "red"
  cryptic_colour <- "grey"
  ...
  allEdges=do.call(rbind,foreach (i=1:nrow(intron_meta)) %do% {
    ...
    edge$verdict <- intron_meta$verdict[i] #ifelse( intron_meta$verdict[i] == "Annotated", yes = "Annotated", no ="Unannotated")
    edge
  })
