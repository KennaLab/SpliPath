#' Count the number of reads support the background junctions of the novel junctions
#'
...
#' @export
countBackGroundJunc <- 
  function(junc_file, junc_anno, output_prefix, annotation = c("in.ens38", "in.snaptron.gtex")){
    ...
    for (gene in genes){
      gene_junc = annoJuncWithShareSites(junc_info, gene)
    ...
    bg_2_read = lapply(bg_ls, function(bg){
      ...
      getJuncCount(junc_file, bg_vec, samples)
    })
