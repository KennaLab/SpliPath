...
#' @export
browserData<-
  function(junc_count, junc_anno, tissues_leafcutter_pvals_file, var2junc, min_read = 5, output_dir = "."){
    ...
    for (n_file in 1:length(junc_count)){
      ...
      sRV = read.table(var2junc[n_file], header=1, stringsAsFactors = F, check.names = F)
      ...
      for (g in 1:nrow(gene_list)){
        ...
        gz_file = gzfile( paste(output_dir, sprintf("%s_%s_crossref.txt.gz", gene_id, gene_name), sep=.Platform$file.sep), "w")
