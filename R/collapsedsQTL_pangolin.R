#' Nominate csQTLs by matching Pangolin predictions of the genetic variants to the provided splicing junctions
#' 
#'
...
#' @param pangolin_prediction list The names of elements in the list is the prediction tool, it should be in GDB database tables. The items are the scoring fields in the GDB database prediction table. The maximum of the given scores will be used as the prediction of the corresponding tool for each variants. E.g. The Pangolin score of a variant will be the maximum of absolute "score_increase", "score_decrease". Default: list(pangolin = c("score_increase", "score_decrease")). 
#' @param min_pangolin_score numeric The min splice score of the prediction tools to nominate rare sQTL candidate. The predictions of splice sites that score higher than the min score will be matched with observed junctions.
...
collapsedsQTL_pangolin <-
  function(junc_anno_file, 
    ...
    pred_field = paste(paste0(names(pangolin_prediction), ".*"), collapse = ", ")
    query = sprintf("select VAR_id, var.CHROM, var.POS, var.REF, var.ALT, %s from var ", pred_field)
    for (item in names(pangolin_prediction)){
    ...
    vars[is.na(vars)] = 0
    for (item in names(pangolin_prediction)){
      vars[, pangolin_prediction[[item]]] = sapply(vars[, pangolin_prediction[[item]]], as.numeric)
      vars[[item]] = apply(abs(vars[, pangolin_prediction[[item]]]), 1, max) ###
      vars = vars[, !colnames(vars) %in% pangolin_prediction[[item]]]
    }
    vars = vars[apply(vars[, names(pangolin_prediction), drop = F], 1, max) > 0, ]
    ...
    vars_gr = vars_gr[, c("seqnames", "start", names(pangolin_prediction), "VAR_id", "DNA_variant", "AF")]
    colnames(vars_gr) = c("chr", "pos", names(pangolin_prediction), "VAR_id", "DNA_variant", "AF")
    ...
    sprintf("var2junc table have %s rows", nrow(var2junc))
    # Match observed junction with Pangolin prediction
    print("Matching variants with junctions ...")
    pangolin_pred_match = matchPredNObs_pangolin(gdb_path = gdb_path, 
                                                 cohort_name = cohort_name,
                                                 var2junc = var2junc, #tissue_map, 
                                                 min_pangolin_score = min_pangolin_score, 
                                                 pangolin_table_name = names(pangolin_prediction),
                                                 max_skip_exon = max_skip_exon,
                                                 reference = reference)
    
    var2junc = dplyr::left_join(var2junc, 
                                pangolin_pred_match[, !colnames(pangolin_pred_match) %in% 
                                                      c("chr", "start", "end", "strand", "start.pos", "end.pos", "MaxDS", "AF", "pangolinPG_idx", "pangolinPL_idx")],
