...
make_variant_plot = function(gdb_path, gene.name, srv_candidate_tbl, chr, start, end, expand_width, plot_xlim){
  ...
  if (nrow(vars) > 0){
    ...
    vars[,c(spliceaiTerm)] = sapply(vars[,c(spliceaiTerm)], FUN=as.numeric)
    vars[is.na(vars)] = 0
  ...
  if (nrow(vars) > 0){
    ...
    csQTL_var = srv_candidate_tbl[srv_candidate_tbl$csQTL_candidate == T, "VAR_id"]
    vars$csQTL_candidate = vars$VAR_id %in% csQTL_var
    
    geno_plot[[i]] = ggplot(vars, aes(x=xaxis, y=SpliceAI, color = csQTL_candidate, label = DNA_variant)) +
      ...
      geom_text(aes(label=ifelse(csQTL_candidate, DNA_variant, '')), vjust=-0.5) + 
      scale_shape_manual(name = 'Allele frequency in GDB < 0.05', values=setNames(c(16, 17), c("F", "T"))) +
      scale_colour_manual(name = 'csQTL candidate', values = setNames(c('#0066CC', 'black'),c(T, F))) +
  ...
  geno_plot[[i]] = geno_plot[[i]] + ggtitle("DNA variants")
...
make_sashimi_like_plot <- function(
  ...
  main_title = paste0("Chr", intron_meta$chr[1], ":", min(s)-expand_width, "-", max(s)+expand_width)
...
plot_splicing = function(dir_path, gdb_path, gene.name, exon_table, gene_table, gene.id=NULL, coord=NULL, gene_wise_only = F){
  ...
  gene_wise = make_gene_wise_plot(exons_gene, novel_junc)
  gene_wise_var = make_variant_plot(gdb_path, gene.name, srv_candidate_tbl, chr, gene_wise$start, gene_wise$end, 0, gene_wise$plot_xlim)
  ...
  if (gene_wise_only == FALSE){
    ...
    novel_junc = novel_junc[novel_junc$start >= plot_start & novel_junc$end <= plot_end, ]

    if (nrow(novel_junc) != 0){
      ### Make RNA splice shashimi-like plots, and get the coordinates of actual plotting region
      #sashimi_plots = make_sashimi_like_plot(subject, exons_table = exon_table, meta = meta, intron_meta = novel_junc, counts = counts[rownames(intron_meta), as.character(meta$SampleID)])
      sashimi_plots = make_gene_wise_plot(exons_gene[(exons_gene$start >= plot_start | exons_gene$end >= plot_start) & (exons_gene$end <= plot_end | exons_gene$start <= plot_end), ], novel_junc)
      local_var = make_variant_plot(gdb_path, gene.name, srv_candidate_tbl, chr, sashimi_plots$start, sashimi_plots$end, sashimi_plots$expand_width, sashimi_plots$plot_xlim)
      # sashimi_plots$plots = c(sashimi_plots$plots, local_var)
      sashimi_plots = c(sashimi_plots, local_var)

    }else{
      sashimi_plots = list()
      sashimi_plots[["plots"]] = list(empty_ggplot_with_info(sprintf("There is no novel splicing junction within %s in gene %s", coord, gene.name)), ggplots())
