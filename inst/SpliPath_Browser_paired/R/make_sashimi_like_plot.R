  ...
  if (length(wgs_id) == 0){
    ...
    for (i in 1:length(wgs_id)){
      ...
      if (nrow(vars) > 0){
        ...
        csQTL_var = srv_candidate_tbl[srv_candidate_tbl$csQTL_candidate == T & srv_candidate_tbl$SubjectID == subject, "VAR_id"]
        ...
        geno_plot[[i]] = ggplot(vars, aes(x=xaxis, y=SpliceAI, color = csQTL_candidate, label = DNA_variant)) +
          ...
          geom_text(aes(label=ifelse(AF<0.05 & SpliceAI >= 0.2 , DNA_variant, '')), vjust=-0.5) + 
...
plot_splicing =  function(dir_path, gdb_path, rna_meta, wgs_meta, subject, gene.name, srv_candidate_tbl, exon_table, gene_table, gene.id=NULL, coord=NULL, gene_wise_only = F){
  ...
  if (gene_wise_only == FALSE){
    ...
    if (nrow(intron_meta) != 0){
      ...
      local_var = make_variant_plot(gdb_path, subject, gene.name, chr, srv_candidate_tbl, sashimi_plots$start, sashimi_plots$end, wgs_id, wgs_meta, sashimi_plots$expand_width, sashimi_plots$plot_xlim)
  ...
  gene_wise_var = make_variant_plot(gdb_path, subject, gene.name, chr, srv_candidate_tbl, gene_wise$start, gene_wise$end, wgs_id, wgs_meta, 0, gene_wise$plot_xlim)
